...
async def self_score(
    ...
    prompt = (
        ...
        "on a CONTINUOUS scale from 1.0 to 5.0 with one decimal place "
        "(e.g. 2.7, 3.4, 4.1), where 1.0=terrible and 5.0=excellent. "
        "Use the full float range to distinguish close cases; whole numbers "
        "are allowed but finer gradations are preferred. "
        "Consider relevance, helpfulness, and naturalness. "
        "Reply with ONLY a JSON object: {\"score\": N.N, \"confidence\": 0.X}\n"
        "<|im_end|>\n"
        "<|im_start|>user\n"
